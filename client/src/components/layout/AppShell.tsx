...
function NavLink({
    ...
    return (
        <Link
            href={href}
            className={cn(
                "group relative flex items-center gap-2 rounded-2xl px-3.5 py-2 text-xs font-black transition-all duration-500",
                active
    ...
    return (
        <div className="min-h-screen bg-background text-slate-900 selection:bg-slate-200">
            ...
            <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-center px-4 pt-4 md:px-6">
                <div className="max-w-7xl w-full bg-white/70 backdrop-blur-xl border border-slate-200/60 h-14 rounded-[24px] px-4 md:px-5 flex items-center justify-between shadow-[0_8px_32px_-8px_rgba(0,0,0,0.06)]">
                    <div className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-800 border border-slate-700">
                            <Languages className="h-4 w-4 text-white" />
                        </div>
                        <span className="font-black text-sm tracking-[0.22em] uppercase text-slate-800">SignTalk</span>
                    </div>
